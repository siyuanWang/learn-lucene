...
public class GoodsIndex {
    public static void main(String[] args) {
        ...
        try {
            ...
            String jsonStr = FileUtils.readFileToString(new File("/Users/wangsiyuan1/workspace/learnlucene/src/main/resources/doc/goods.json"));
            JSONArray array = JSON.parseArray(jsonStr);
            for (int i = 0, length = array.size(); i < length; i++) {
                JSONObject json = array.getJSONObject(i);

                Document doc = new Document();
                doc.add(new TextField("title", json.getString("title"), Field.Store.YES));
                doc.add(new LongPoint("skuId", json.getLong("skuId")));
                doc.add(new IntPoint("cid1", json.getInteger("cid1")));
                doc.add(new StringField("cid1Name", json.getString("cid1Name"), Field.Store.YES));
                doc.add(new IntPoint("cid2", json.getInteger("cid2")));
                doc.add(new StringField("cid2Name", json.getString("cid2Name"), Field.Store.YES));
                doc.add(new IntPoint("cid3", json.getInteger("cid3")));
                doc.add(new StringField("cid3Name", json.getString("cid3Name"), Field.Store.YES));
